...
<tr class='row-container row_<?php echo $position; ?>' id='row_<?php echo $language; ?>_<?php echo $position; ?>_<?php echo $qid; ?>_<?php echo $scale_id; ?>' data-common-id="<?php echo $position; ?>_<?php echo $qid; ?>_<?php echo $scale_id; ?>">
    ...
    <!-- feedback -->
    <?php if ($assessmentvisible && $first): ?>
        <td class="subquestion-text">
        <select  
            class='form-control'
            id='feedback_type_<?php echo $position; ?>_<?php echo $scale_id; ?>'
            name='feedback_type_<?php echo $position; ?>_<?php echo $scale_id; ?>'
        >
            <option value="">None</option>
            <option value="praise" <?php if(isset($feedback_type) && $feedback_type=="praise"){echo "selected";} ?> >praise</option>
            <option value="remedy" <?php if(isset($feedback_type) && $feedback_type=="remedy"){echo "selected";} ?> >remedy</option>
            <option value="moderate" <?php if(isset($feedback_type) && $feedback_type=="moderate"){echo "selected";} ?>>moderate</oprion>
            <option value="feedback" <?php if(isset($feedback_type) && $feedback_type=="feedback"){echo "selected";} ?>>feedback</option>
        </select>
        <!--
            <input
                type='text'
                ...
                id='feedback_type_<?php echo $position; ?>_<?php echo $scale_id; ?>'
                name='feedback_type_<?php echo $position; ?>_<?php echo $scale_id; ?>'
                value="<?php if(isset($feedback_type)){echo $feedback_type;} ?>"
            />
            -->
        </td>
        <td class="subquestion-text">
        <div class="input-group feedback-field">
            <input
                type='text'
                class='feedback form-control input'
                id='feedback_content_<?php echo $position; ?>_<?php echo $scale_id; ?>'
                name='feedback_content_<?php echo $position; ?>_<?php echo $scale_id; ?>'
                value="<?php if(isset($feedback_content)){echo $feedback_content;} ?>"
            />
            <span class="input-group-addon">
                <?php echo  getEditor("editanswer","feedback_content_".$qid."_{$scale_id}", "[".gT("Feedback Content:", "js")."](".$language.")",$surveyid,$gid,$qid,'editanswer'); ?>
            </span>
        </div>
        </td>
    <?php endif; ?>
