...
class SurveyRuntimeHelper
{
    ...
    public function run($surveyid, $args)
    {
        ...
        $session = $_SESSION[$this->LEMsessid];

        $myMenu = $session["grouplist"];
        foreach($myMenu as &$group){
            $group["questions"] = array();
            foreach($session["fieldarray"] as $field){
                if($field[5]==$group["gid"]){
                    array_push($group["questions"], array("question"=> $field[3], "qid"=>$field[2], "gid"=>$field[5]));
                }
            }
        }
...
// Use survey level settings
     ...
     * 获取整体报告
     */
    private function getReport($session){
        //计算IDK
        $idk=array();
        $idk_total = 0;
        foreach($this->getReportFields($session) as $_key=>$field){
            if(strtoupper($session[$field["fieldname"]])=="IDK"){
                $idk_total += 1;
                if(isset($idk[$field["gid"]])){
                    $idk[$field["gid"]] += 1;
                }else{
                    $idk[$field["gid"]] = 1;
                }
        ...
        //Report 报告评分生成
        $myReport = array();
        foreach($session["grouplist"] as $my_group){
            $my_gid = $my_group["gid"];
            if(!isset($this->aSurveyInfo['aAssessments']['datas']['group'][$my_gid])){
                continue;
            }
            //分组规则
            $my_group["assessments"]=$this->aSurveyInfo['aAssessments']['datas']['group'][$my_gid];
            //当前组得分
            $my_group["score"]=$this->aSurveyInfo['aAssessments']['datas']['subtotal_score'][$my_gid];
            if(isset($idk[$my_gid])){
                $my_group["IDK"] = $idk[$my_gid];
            }else{
                $my_group["IDK"] = 0;
            }
            array_push($myReport, $my_group); 
        }
        $myReportByTotal = array("assessments"=>array());
        if(isset($this->aSurveyInfo['aAssessments']['datas']['total'])){
            foreach($this->aSurveyInfo['aAssessments']['datas']['total'] as $k=>$val){
                if(is_array($val)){
                    $myReportByTotal["assessments"][$k]=$val;
                }
            }
        }
        if(isset($this->aSurveyInfo['aAssessments']['datas']['total_score'])){
            $myReportByTotal["score"] = $this->aSurveyInfo['aAssessments']['datas']['total_score'];
        }else{
            $myReportByTotal["score"] = 0;
        }
        $myReportByTotal["IDK"] = $idk_total;

        return array("group"=>$myReport, "total"=>$myReportByTotal);
    }
    private function getReportFields($session){
        /**
         * 从会话中获取需要报告的题目数据项
         * 只处理以下三种数据类型
         * List with comment=O, List(radio)=L, Array=F
         */
        $res = array();
        foreach($session["fieldarray"] as $name=>$values){
            if(in_array($values[4], array('F', 'L', 'O'))){
                foreach($session["fieldnamesInfo"] as $k=>$val){
                    if($name==$val){
                        $res[$k]=$session["fieldmap"][$k];
                    }
                }
            }
        }
    ...
    /**
     * 获取每题报告feedback_content
     * 处理以下三种数据类型
     * List with comment=O, List(radio)=L, Array=F
     */
     private function getSubReport($session){
        $mySubReport = array("praise"=>array(), "moderate"=>array(), "remedy"=>array(), "feedback"=>array());
        foreach($this->getReportFields($session) as $_key=>$field){
            //$field = $session["fieldmap"][$_key];
            $field["answer_code"] = $session[$_key];
            if(!isset($field["subquestion"])){
                $field["subquestion"] = "";
            }
            $field['tips_answer_start'] = "";
            // 替换引用
            $field['question'] = LimeExpressionManager::ProcessString($field['question']);
            /*
            $query = "SELECT * FROM {{answers}}
            WHERE qid=".$field["qid"]." and language='".$session['s_lang']."'
                ORDER BY sortorder";
            */
            // 去除languge
            $query = "SELECT * FROM {{answers}} WHERE qid=".$field["qid"]." ORDER BY sortorder";

            if ($result = dbExecuteAssoc($query)) {
                $aResultSet = $result->readAll();
                if (count($aResultSet) > 0) {
                    foreach($aResultSet as $i=>$answer){
                        if($i==0){
                            $field['tips_answer_start'] = $answer["answer"];
                        }elseif(strtoupper($answer['code'])!="IDK"){
                            $field['tips_answer_end'] = $answer["answer"];    
                        }
                            
                        if($answer['code']==$field["answer_code"]){
                            $field["answer"] = $answer["answer"];
                            $field["feedback_type"] = strtolower($answer["feedback_type"]);
                            $field["feedback_content"] = $answer["feedback_content"];
                        }
                    }
                }
            }
            if(isset($field["feedback_type"])){
                $mySubReport[$field["feedback_type"]][$_key] = $field;
            }
        }
        return $mySubReport;
     }
    private function dump($value, $tab=4){
        /**
         * 格式化显示变量
         */
        echo "<br />";
        foreach($value as $key=>$val){
            echo str_repeat("&nbsp", $tab).$key."&nbsp;=&nbsp;";
            if(is_array($val)){
                $this->dump($val, $tab+$tab);
            }else{
                var_dump($val);
                echo "<br>";
            }
    ...
    private function moveSubmitIfNeeded()
    {
        if ($this->sMove == "movesubmit") {
            ...
            if ($this->aSurveyInfo['active'] != "Y") {
                ...
                // 非激活调查，模拟提交
                $session = $_SESSION[$this->LEMsessid];
                //$this -> dump($this->getReport($session));
                Yii::app()->twigRenderer->renderTemplateFromFile("layout_global.twig", array('oSurvey'=> Survey::model()->findByPk($this->iSurveyid), 'aSurveyInfo'=>$this->aSurveyInfo, "aSubReport"=>$this->getSubReport($session), 'aReport'=>$this->getReport($session)), false);
            } else {

                //Update the token if needed and send a confirmation email
                
                if (isset($_SESSION['survey_'.$this->iSurveyid]['token'])) {
                    $tokeninfo = $_SESSION['survey_'.$this->iSurveyid];
                    //submittokens();
                }
            ...
            $myReport = $this->getReport($session);
            // 计算每题答案
            $mySubReport = $this->getSubReport($session);
            // 单体评分
            //$aAssessments = array();
            //发送邮件
            //Update the token if needed and send a confirmation email
            ...
            if (isset($tokeninfo)) {
                ...
                $sHtml = Yii::app()->twigRenderer->renderTemplateFromFile("layout_global.twig", array('oSurvey'=> Survey::model()->findByPk($this->iSurveyid), 'aSurveyInfo'=>$this->aSurveyInfo, "aSubReport"=>$this->getSubReport($session), 'aReport'=>$this->getReport($session), 'noJs'=>True), true);
                $attachment_file = getcwd()."/reports/reports-".$token.".html";
                $reportfile = fopen($attachment_file, "w") or die("Unable to open file!");
                fwrite($reportfile, $sHtml);
                fclose($reportfile);
                //var_dump($tokeninfo);
                //将调查添加回$_SESSION
                $_SESSION['survey_'.$this->iSurveyid] = $tokeninfo;
                submittokens(false, $attachment_file);
                //删除添加的调查信息
                unset($_SESSION['survey_'.$this->iSurveyid]);
            }
                

            
            Yii::app()->twigRenderer->renderTemplateFromFile("layout_global.twig", array('oSurvey'=> Survey::model()->findByPk($this->iSurveyid), 'aSurveyInfo'=>$this->aSurveyInfo, 'aReport'=>$myReport, 'aSubReport'=>$mySubReport), false);
        }
    }
