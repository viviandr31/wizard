...
class database extends Survey_Common_Action
{
    ...
    private $updateableFields = [
                ...
                'emailccto' => ['type'=> '', 'default' => false, 'dbname'=>false, 'active'=>true, 'required'=>[]],
    ...
    private function actionUpdateAnswerOptions($iSurveyID)
    {
        ...
        for ($iScaleID = 0; $iScaleID < $iScaleCount; $iScaleID++) {
            ...
            for ($iSortOrderID = 1; $iSortOrderID < $iMaxCount; $iSortOrderID++) {
                ...
                $sFeedbackType =  Yii::app()->request->getPost('feedback_type_'.$iSortOrderID.'_'.$iScaleID);
                $sFeedbackContent = Yii::app()->request->getPost('feedback_content_'.$iSortOrderID.'_'.$iScaleID);
                ...
                foreach ($survey->allLanguages as $sLanguage) {
                    ...
                    $oAnswer->feedback_type     = $sFeedbackType;
                    $oAnswer->feedback_content  = $sFeedbackContent;
...
//new record: additional language
    ...
    private function actionUpdateSurveyLocaleSettings($iSurveyID)
    {
        ...
        if (Permission::model()->hasSurveyPermission($iSurveyID, 'surveylocale', 'update')) {
            foreach ($languagelist as $langname) {
                if ($langname) {
                    ...
                    $report_header = Yii::app()->request->getPost('report_header', null);
                    $report_footer = Yii::app()->request->getPost('report_footer', null);
                    ...
                    if ($report_header !== null) {
                        // Fix bug with FCKEditor saving strange BR types
                        $report_header = $this->oFixCKeditor->fixCKeditor($report_header);
                        $data['report_header'] = $report_header;
                    }
                    if ($report_footer !== null) {
                        // Fix bug with FCKEditor saving strange BR types
                        $report_footer = $this->oFixCKeditor->fixCKeditor($report_footer);
                        $data['report_footer'] = $report_footer;
                    }
                    if (count($data) > 0) {
        ...
        if (Permission::model()->hasSurveyPermission($iSurveyID, 'surveysettings', 'update')) {
            ...
            $oSurvey->emailccto = $this->_filterEmptyFields($oSurvey, 'emailccto');
