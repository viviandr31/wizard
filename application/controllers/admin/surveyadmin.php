    ...
    public function insert($iSurveyID = null)
    {
        if (Permission::model()->hasGlobalPermission('surveys', 'create')) {
            ...
            $aInsertData = array(
                ...
                'emailccto' => App()->request->getPost('emailccto'),
            ...
            $sReportHeader   = Yii::app()->request->getPost('report_header','');
            $sReportFooter   = Yii::app()->request->getPost('report_footer','');
            ...
            $sReportHeader   = html_entity_decode($sReportHeader, ENT_QUOTES, "UTF-8");
            $sReportFooter   = html_entity_decode($sReportFooter, ENT_QUOTES, "UTF-8");

            // Fix bug with FCKEditor saving strange BR types
            $sTitle       = fixCKeditorText($sTitle);
            $sDescription = fixCKeditorText($sDescription);
            $sWelcome     = fixCKeditorText($sWelcome);
            $sReportHeader= fixCKeditorText($sReportHeader);
            $sReportFooter= fixCKeditorText($sReportFooter);
            ...
            $aInsertData = array(
                ...
                'report_header'           => $sReportHeader,
                'report_footer'           => $sReportFooter,
            );
